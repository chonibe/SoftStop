# SoftStop brand

## Mark

Soft halt bar through a geometric **S** — permit/block without looking like a hard stop light or security shield. Background is always transparent.

- Ink: `#0B0B0F`
- Paper: `#F7F5F0`
- Stop amber: `#E8A317`

## Assets

| File | Use |
|------|-----|
| [softstop-mark.png](softstop-mark.png) | Default / light backgrounds (ink S) |
| [softstop-mark-dark.png](softstop-mark-dark.png) | Dark mode / dark backgrounds (paper S) |
| [softstop-mark.svg](softstop-mark.svg) | Vector — light |
| [softstop-mark-dark.svg](softstop-mark-dark.svg) | Vector — dark |
| [softstop-mark-1024.png](softstop-mark-1024.png) / [softstop-mark-dark-1024.png](softstop-mark-dark-1024.png) | High-res |
| [softstop-wordmark.svg](softstop-wordmark.svg) | README / demo header |
| [softstop-cover.png](softstop-cover.png) | Social / README hero (~1200×630) |
| [softstop-before-after.png](softstop-before-after.png) | Story proof |
| [softstop-cover.svg](softstop-cover.svg) / [softstop-before-after.svg](softstop-before-after.svg) | Vector sources |

## Do

- Flat, one accent color
- SoftStop as one word
- Transparent mark backgrounds (no filled tile)
- Pressure permit / stop-when-it-should framing

## Don't

- Purple AI gradients, glow orbs, baby soft-spot literalism
- Shield/lock IAM vibes
- Opaque white or black square behind the mark
- MCP / agent-firewall framing on the mark
